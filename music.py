...
def to_vector_ts(score: stream.Score, hasParts = True):
    ...
    note_endings = [note.offset + note.duration.quarterLength for note in score.recurse().getElementsByClass('Note')]
    score_duration = max(note_endings)
    # Multiplied by 4, because we want it in 16th notes rather than quarter notes:
    output_ts = np.zeros([int(score_duration * 4 + 1), len(score.getElementsByClass("Part"))])
